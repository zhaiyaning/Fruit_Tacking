...
class KalmanFilterXYAH:
    ...
    def update(self, mean, covariance, measurement) -> tuple:
        ...
        """
        # 线性化观测函数
        # 假设我们使用直接的观测模型 (x, y, w, h) 作为观测值，因此
        # 我们的观测函数是直接从状态空间提取位置和大小。
        jacobian_measurement = np.zeros((4, 8))

        # 对应的雅可比矩阵，提取位置和宽高
        jacobian_measurement[0:4, 0:4] = np.eye(4)  # 位置 (x, y, w, h) 直接映射

        # 投影到观测空间
        projected_mean = np.dot(jacobian_measurement, mean)
        projected_cov = np.linalg.multi_dot((jacobian_measurement, covariance, jacobian_measurement.T))

        # 计算卡尔曼增益
        chol_factor, lower = scipy.linalg.cho_factor(projected_cov, lower=True, check_finite=False)
        kalman_gain = scipy.linalg.cho_solve(
            (chol_factor, lower), np.dot(covariance, jacobian_measurement.T).T, check_finite=False
        ).T
...
class KalmanFilterXYWH(KalmanFilterXYAH):
    ...
    def predict(self, mean, covariance) -> tuple:
        ...
        """
        # 线性化状态转移函数
        # 计算雅可比矩阵，这里可以通过计算状态转移函数对位置和速度的偏导数来得到
        # 假设我们使用常规的线性运动模型 x(t+1) = x(t) + v(t) * dt
        jacobian_motion = np.eye(8)  # 假设线性状态转移

        jacobian_motion[0:4, 4:8] = np.eye(4)  # 为了包括速度对位置的影响

        # 更新均值 (状态转移方程)
        mean = np.dot(jacobian_motion, mean)
        ...
        covariance = np.linalg.multi_dot((jacobian_motion, covariance, jacobian_motion.T)) + motion_cov
    ...
    def multi_predict(self, mean, covariance) -> tuple:
        ...
        Run Kalman filter prediction step (Vectorized version).
        """
        jacobian_motion = np.eye(8)
        jacobian_motion[0:4, 4:8] = np.eye(4)  # For velocity-to-position impact

        # Update mean (state transition)
        mean = np.dot(mean, jacobian_motion.T)  # Make sure mean shape matches

        # Standard deviations for positions and velocities
        std_pos = [
            self._std_weight_position * mean[:, 2],
            self._std_weight_position * mean[:, 3],
            self._std_weight_position * mean[:, 2],
        ...
        sqr = np.square(np.r_[std_pos, std_vel]).T
        motion_cov = [np.diag(sqr[i]) for i in range(len(mean))]
        motion_cov = np.array(motion_cov)

        # Update covariance
        left = np.dot(jacobian_motion, covariance).transpose((1, 0, 2))
        covariance = np.dot(left, jacobian_motion.T) + motion_cov
