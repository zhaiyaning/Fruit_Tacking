...
class InitalKalmanFilterXYWH(KalmanFilterXYAH):
    """
...
class KalmanFilterXYWH(InitalKalmanFilterXYWH):
    def __init__(self, lambda_min=0.95, lambda_max=1.0, gamma=1.0, jitter=1e-6):
        """
        Args:
            lambda_min (float): Minimum forgetting factor.
            lambda_max (float): Maximum forgetting factor.
            gamma (float): Scale factor for exponential adjustment based on innovation norm.
            jitter (float): Small value added to diagonal of covariance for numerical stability.
        """
        super().__init__()
    ...
    def compute_forgetting_factor(self, innovation: np.ndarray) -> float:
        norm_squared = np.dot(innovation, innovation)
        forgetting_factor = (
            self.lambda_min
            + (self.lambda_max - self.lambda_min) * np.exp(-self.gamma * norm_squared)
        )
        return forgetting_factor

    def ensure_positive_definite(self, matrix: np.ndarray) -> np.ndarray:
        """
        Ensure the input matrix is positive definite by adding jitter to the diagonal if needed.

        Args:
            matrix (ndarray): Covariance matrix to check and adjust.

        Returns:
            ndarray: Adjusted positive-definite covariance matrix.
        """
        try:
            ...
            if min_eigenvalue < 0:
                matrix += (-min_eigenvalue + self.jitter) * np.eye(matrix.shape[0])
            else:
                matrix += self.jitter * np.eye(matrix.shape[0])
        return matrix
    ...
    def update(self, mean: np.ndarray, covariance: np.ndarray, measurement: np.ndarray) -> tuple:
        ...
        innovation = measurement - projected_mean
        forgetting_factor = self.compute_forgetting_factor(innovation)

        # Ensure projected covariance is positive definite
        projected_cov = self.ensure_positive_definite(projected_cov)
        ...
        new_covariance = (
            np.dot(identity - np.dot(kalman_gain, self._update_mat), covariance) / forgetting_factor
            + (1 - 1 / forgetting_factor) * covariance
        )

        # Ensure new covariance is positive definite
        new_covariance = self.ensure_positive_definite(new_covariance)
