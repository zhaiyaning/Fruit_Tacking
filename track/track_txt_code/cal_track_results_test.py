...
def calculate_mota(tracking_results, ground_truth):
    ...
    total_frames = min(len(tracking_results), len(ground_truth))
    tracking_results = tracking_results[:total_frames]
    ground_truth = ground_truth[:total_frames]

    #assert total_frames == len(ground_truth), "跟踪结果和真实轨迹必须具有相同的帧数。"
    assert total_frames == len(ground_truth), f"Tracking results have {total_frames} frames, but ground truth has {len(ground_truth)} frames."
    ...
    previous_gt_ids = set()  # 上一帧的真实轨迹ID集合
    ...
    for frame in range(total_frames):
        ...
        current_tracked_ids = set(tracked_ids)

        # 对比当前帧与上一帧的ID进行ID切换检测
        if previous_gt_ids:
            # 找出在当前帧与上一帧之间的ID切换
            switched_ids = previous_gt_ids.symmetric_difference(current_gt_ids)
            IDSW += len(switched_ids)

        previous_gt_ids = current_gt_ids  # 更新上一帧的真实轨迹ID集合
...
for n in n_values:
    ...
    # 保存每次计算的结果
    mota_results.append(mota_value )  
    idf1_results.append(idf1_value)
    hota_results.append(hota_value )  
