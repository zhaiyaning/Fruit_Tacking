...
def plot_tracking_data(frame_numbers1, tracking_results1, frame_numbers2, tracking_results2,
                       frame_numbers3, tracking_results3, frame_numbers4, tracking_results4,
                       frames_mot, object_counts_mot):
    plt.figure(figsize=(12, 6))

    # 绘制每个数据集，使用不同的线型、标记和线宽
    plt.plot(frame_numbers1, tracking_results1, linestyle='-', color='b', markersize=6, linewidth=2,
             label='Dataset 1: botsort_EMF')
    plt.plot(frame_numbers2, tracking_results2, linestyle='--', color='r', markersize=6, linewidth=2,
             label='Dataset 2: botsort_v8')
    plt.plot(frame_numbers3, tracking_results3, linestyle='-.', color='g',  markersize=6, linewidth=2,
             label='Dataset 3: bytetrack_EMF')
    plt.plot(frame_numbers4, tracking_results4, linestyle=':', color='c',  markersize=6, linewidth=2,
             label='Dataset 4: bytetrack_V8')

    # 绘制MOT数据集的折线图
    plt.plot(frames_mot, object_counts_mot, linestyle='-', color='m', markersize=6, linewidth=2,
             label='MOT Dataset')

    plt.title('Number of Detections Over Frames')
...
# 获取数据
frame_numbers1, tracking_results1 = get_track_data(input_path=r'E:\Astudy\mycode\test_track\track_txt\botsort_EMF.txt')
frame_numbers2, tracking_results2 = get_track_data(input_path=r'E:\Astudy\mycode\test_track\track_txt\botsort_v8.txt')
frame_numbers3, tracking_results3 = get_track_data(
    input_path=r'E:\Astudy\mycode\test_track\track_txt\bytetrack_EMF.txt')
frame_numbers4, tracking_results4 = get_track_data(input_path=r'E:\Astudy\mycode\test_track\track_txt\bytetrack_V8.txt')
...
plot_tracking_data(frame_numbers1, tracking_results1, frame_numbers2, tracking_results2, frame_numbers3,
                   tracking_results3, frame_numbers4, tracking_results4, frames_mot, object_counts_mot)


# Calculate the Fréchet distances
distances = {
    'botsort_EMF vs GT': frechet_distance(np.array(tracking_results1), np.array(tracking_results4)),
    'botsort_V8 vs GT': frechet_distance(np.array(tracking_results2), np.array(tracking_results4)),
}
