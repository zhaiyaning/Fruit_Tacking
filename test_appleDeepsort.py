...
# cap.set(3, 1280)
# cap.set(4, 720)
cap = cv2.VideoCapture(r"E:\Astudy\yolov8\ultralytics-main\10_output_video.mp4")  # 对于视频

# 加载 YOLO 模型
model = YOLO(r"E:\Astudy\yolov8\ultralytics-main\weights\appletree_v8.pt")  # 使用较大的模型以获得更好的 GPU 性能
...
output_video_path = r'E:\Astudy\mycode\test_track\all_trackresult\track_video\v8_DeepSort.avi'  # Output video file path
...
while True:
    ...
    for track in tracks:
        ...
        if currentClass == "apple":
            clsColor = (0, 0, 255)  # Red for apple


        # 在帧上绘制跟踪信息
        #cvzone.cornerRect(img, (x1, y1, w, h), l=9, rt=2, co0lorR=clsColor)
        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 2)
        text = f"ID: {track_id}, apple {conf:.2f}"
    ...
    # 显示苹果计数
        #cvzone.putTextRect(img, text=f"ID: {track_id}, apple {conf:.2f}", pos=text_position,
         #              scale=1, thickness=1, offset=2, colorR=(0, 255, 0))
